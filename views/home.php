...
$tipHash = lx_tip_hash($net);
$blocks  = lx_recent_blocks($net, null, 10);
...
$recentTxs = lx_mempool_recent($net);
$supply    = lx_supply_info($net);
$dist      = lx_mining_distribution($net);   // mempool.space-style pool snapshot for the home
$mwebTot   = lx_mweb_enabled($net) ? lx_mweb_peg_totals($net) : null;
...
$strip = lx_recent_block_stats($net, 12);
$proj  = lx_projected_blocks($net);
...
lx_head($net, [
    'title' => $net['label'] . ' Explorer',
    'desc'  => $net['label'] . ' block explorer with MWEB support · chain tip '
             . commas($tip) . ' · ' . commas($mem['count']) . ' transactions in the mempool · blocks, transactions, addresses, mining and fees, live from the node.',
    'og_image' => '/og/home.png',
]);

// Live LTC price (best-effort; hides if the source is unavailable/disabled).
$ltcPriceNow = lx_price_now($net);                       // float|null
$ltcChg      = $ltcPriceNow !== null ? lx_price_change($net) : null;
$priceCode   = strtoupper(lx_price_config()['display']);
$priceSym    = lx_price_symbol(lx_price_config()['display']);
$ltcPriceStr = $ltcPriceNow !== null ? $priceSym . number_format($ltcPriceNow, $ltcPriceNow < 1 ? 4 : 2) : null;
?>
...
<div class="card hero">
  <div class="card-b between">
    <div>
      ...
      <div class="big-num" id="live-tip" data-tip="<?= h(lx_u($net)) ?>/api/blocks/tip">#<?= commas($tip) ?></div>
      <div class="muted sub"><a class="addr" id="live-tiphash" href="<?= h(lx_block_href($net, $tipHash)) ?>"><?= h($tipHash) ?></a></div>
    </div>
    <div class="stat-mini">
      ...
      <div><span class="muted">Hashrate</span><b><?= h(lx_hashrate($hashrate)) ?></b></div>
      <div><span class="muted">Difficulty</span><b><?= h($diffStr) ?></b></div>
      <?php if ($ltcPriceStr !== null): ?><div><span class="muted">Price</span><b><span id="live-price" data-cur="<?= h($priceCode) ?>" data-sym="<?= h($priceSym) ?>"><?= h($ltcPriceStr) ?></span><?php if ($ltcChg !== null): ?> <span id="live-change" class="<?= $ltcChg >= 0 ? 'pos' : 'neg' ?>"><?= ($ltcChg >= 0 ? '+' : '') . h(number_format($ltcChg, 2)) ?>%</span><?php endif; ?></b></div><?php endif; ?>
    </div>
  </div>
</div>
...
<div class="card">
  ...
  <div class="card-b nopad">
    <div class="blocks-strip" id="blocks-strip">
      ...
      <a class="blk proj" href="<?= h(lx_u($net)) ?>/mempool-block/<?= (int) $pi ?>">
      ...
      <?php foreach ($strip as $b): $bc = lx_feerate_color($b['med_feerate']); ?>
...
<?php if ($fees): ?>
<div class="card">
  <div class="card-h"><span><?= lx_icon('trending-up') ?>Fee estimates</span> <span class="sub">sat/vB</span></div>
  <div class="card-b">
    <div class="stat-grid">
      <div class="stat" style="border-left:3px solid <?= h(lx_feerate_color((float) $fees['minimumFee'])) ?>"><div class="muted sub">No priority</div><div class="big-num sm" style="color:<?= h(lx_feerate_color((float) $fees['minimumFee'])) ?>"><?= commas($fees['minimumFee']) ?></div></div>
      <div class="stat" style="border-left:3px solid <?= h(lx_feerate_color((float) $fees['economyFee'])) ?>"><div class="muted sub">Economy</div><div class="big-num sm" style="color:<?= h(lx_feerate_color((float) $fees['economyFee'])) ?>"><?= commas($fees['economyFee']) ?></div></div>
      <div class="stat" style="border-left:3px solid <?= h(lx_feerate_color((float) $fees['hourFee'])) ?>"><div class="muted sub">~1 hour</div><div class="big-num sm" style="color:<?= h(lx_feerate_color((float) $fees['hourFee'])) ?>"><?= commas($fees['hourFee']) ?></div></div>
      <div class="stat" style="border-left:3px solid <?= h(lx_feerate_color((float) $fees['halfHourFee'])) ?>"><div class="muted sub">~30 min</div><div class="big-num sm" style="color:<?= h(lx_feerate_color((float) $fees['halfHourFee'])) ?>"><?= commas($fees['halfHourFee']) ?></div></div>
      <a class="stat hot" href="<?= h(lx_u($net)) ?>/mempool-block/0" style="border-left:3px solid <?= h(lx_feerate_color((float) $fees['fastestFee'])) ?>;text-decoration:none;color:inherit" title="View the projected next block"><div class="muted sub">Next block</div><div class="big-num sm" style="color:<?= h(lx_feerate_color((float) $fees['fastestFee'])) ?>"><?= commas($fees['fastestFee']) ?></div></a>
    </div>
  </div>
</div>
<?php endif; ?>
...
<div class="card">
  <div class="card-h"><span><?= lx_icon('activity') ?>Network</span> <a class="sub" href="<?= h(lx_u($net)) ?>/mining">Mining &amp; graphs &rarr;</a></div>
  <div class="card-b">
    <div class="stat-grid">
      <div class="stat"><div class="muted sub"><?= lx_icon('activity') ?>Difficulty</div><div class="big-num sm"><?= h($diffStr) ?></div></div>
      <div class="stat"><div class="muted sub"><?= lx_icon('zap') ?>Hashrate</div><div class="big-num sm"><?= h(lx_hashrate($hashrate)) ?></div></div>
      <?php if ($da): ?>
      <div class="stat"><div class="muted sub"><?= lx_icon('clock-sm') ?>Avg block time</div><div class="big-num sm"><?= h(number_format($da['timeAvg'] / 60000, 1)) ?> min</div></div>
      <div class="stat"><div class="muted sub"><?= lx_icon('target') ?>Next retarget</div><div class="big-num sm"><?= h(number_format($da['progressPercent'], 0)) ?>%</div><div class="muted sub"><?= commas($da['remainingBlocks']) ?> left &middot; ~<?= h(gmdate('M j', (int) ($da['estimatedRetargetDate'] / 1000))) ?></div></div>
      <div class="stat"><div class="muted sub"><?= lx_icon('trending-up') ?>Est. difficulty change</div><div class="big-num sm <?= $da['difficultyChange'] >= 0 ? 'pos' : 'neg' ?>"><?= ($da['difficultyChange'] >= 0 ? '+' : '') . h(number_format($da['difficultyChange'], 2)) ?>%</div><div class="muted sub">prev <?= ($da['previousRetarget'] >= 0 ? '+' : '') . h(number_format($da['previousRetarget'], 2)) ?>%</div></div>
      <?php endif; ?>
...
<?php if ($supply): ?>
<div class="card">
  <div class="card-h"><span><?= lx_icon('database') ?>Supply &amp; issuance</span> <span class="sub"><?= h($net['unit']) ?></span></div>
  <div class="card-b">
    <div class="stat-grid">
...
?>
<div class="card">
  <div class="card-h"><span><?= lx_icon('cpu') ?>Mining pools</span> <span class="sub">last <?= (int) $dist['window'] ?> blocks</span></div>
  <div class="card-b">
    <div class="stat-grid">
      <div class="stat"><div class="muted sub"><?= lx_icon('zap') ?>Top miner</div><div class="big-num sm"><?= h($lead['name']) ?></div><div class="muted sub"><?= h(number_format($lead['pct'], 1)) ?>% of last <?= (int) $dist['window'] ?></div></div>
      <div class="stat"><div class="muted sub"><?= lx_icon('activity') ?>Miners</div><div class="big-num sm"><?= commas(count($dist['pools'])) ?></div><div class="muted sub">seen in window</div></div>
    </div>
    ...
    <table>
      ...
      <?php foreach ($ptop as $i => $p): ?>
        <tr>
          ...
          <td><a href="<?= h(lx_u($net)) ?>/mining/<?= h(rawurlencode($p['name'])) ?>"><?= h($p['name']) ?></a></td>
  ...
  <div class="card-b"><a class="sub" style="display:inline-flex;align-items:center;gap:7px" href="<?= h(lx_u($net)) ?>/mining"><?= lx_icon('cpu') ?>All miners &amp; hashrate &rarr;</a></div>
</div>
<?php endif; ?>
...
<?php if ($feeBands): ?>
<div class="card">
  <div class="card-h"><span><?= lx_icon('layers') ?>Mempool congestion</span> <a class="sub" href="<?= h(lx_u($net)) ?>/mempool">sat/vB &rarr; vsize</a></div>
  <div class="card-b">
...
<div class="txio">
  <div class="card">
    <div class="card-h"><span><?= lx_icon('box') ?>Latest blocks</span> <a class="sub" href="<?= h(lx_u($net)) ?>/blocks">View all →</a></div>
    <div class="card-b nopad table-wrap">
      <table>
  ...
  <div class="card">
    <div class="card-h"><span><?= lx_icon('repeat') ?>Latest transactions</span> <a class="sub" href="<?= h(lx_u($net)) ?>/mempool">Mempool →</a></div>
    <div class="card-b nopad table-wrap">
      <table>
        ...
        <?php endforeach; ?>
        <?php if (!$recentTxs): ?><tr><td colspan="3"><div class="empty"><?= lx_icon('inbox') ?><span>Mempool is empty.</span></div></td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </div>
