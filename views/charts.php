...
ts_head($net, ['title' => 'Charts - ' . $net['label'] . ' Explorer']);
...
if ($biSeries) {
    foreach ($biSeries as $b) {
        ...
        $fsTips[]   = ts_tip_json('Block ~#' . number_format((int) $b['avgHeight']), [
...
if ($tipH > 0 && ($net['coin'] ?? '') === 'ltc') {
    ...
    for ($i = 0; $i <= 4; $i++) { $supplyXticks[] = ['f' => $i / 4, 'label' => '#' . ts_num_compact($tipH * $i / 4)]; }
}

$priceRows = []; $priceLabels = []; $priceXticks = [];
$pcur = strtoupper(ts_price_config()['display']);
$phist = function_exists('ts_price_history') ? ts_price_history($net) : ['prices' => []];
foreach (($phist['prices'] ?? []) as $pp) {
    if (isset($pp[$pcur])) {
        $priceRows[]   = ['t' => (int) $pp['time'], 'p' => (float) $pp[$pcur]];
        $priceLabels[] = gmdate('M j', (int) $pp['time']) . ' · ' . ts_price_symbol(strtolower($pcur)) . number_format((float) $pp[$pcur], 2);
    }
}
...
if (count($priceRows) < 2 && function_exists('ts_price_market_chart')) {
    $priceLabels = [];
    foreach (ts_price_market_chart($net, $pcur, 365) as $pp) {
        $priceRows[]   = $pp;
        $priceLabels[] = gmdate('M j', $pp['t']) . ' · ' . ts_price_symbol(strtolower($pcur)) . number_format($pp['p'], 2);
    }
}
...
<?php if (!empty($epochs)):
    ...
    foreach (array_reverse($epochs) as $e) {
        ...
        $epochTips[]   = ts_tip_json(gmdate('Y-m-d', (int) $e['time']) . ' · retarget #' . number_format((int) $e['height']), [
            ['c' => $pct >= 0 ? 'var(--ok)' : 'var(--bad)', 'k' => 'Change',     'v' => ($pct >= 0 ? '+' : '') . number_format($pct, 2) . '%'],
            ['c' => 'var(--muted)',                          'k' => 'Difficulty', 'v' => ts_diff_str((float) $e['difficulty'])],
        ]);
    }
?>
<div class="card">
  <div class="card-h"><span><?= ts_icon('target') ?>Difficulty adjustments</span> <span class="sub">per 2,016-block retarget · %</span></div>
...
<div class="txio">
  ...
  <div class="card">
    <div class="card-h"><span><?= ts_icon('database') ?>Coin supply</span> <span class="sub">issued · <?= h($net['unit']) ?></span></div>
    <div class="card-b"><?= ts_chart_area($supplyRows, 'h', 'supply', 'Litecoin supply issued over time', $supplyLabels, [
  ...
  <div class="card">
    <div class="card-h"><span><?= ts_icon('trending-up') ?>Price</span> <span class="sub"><?= h($pcur) ?> · history</span></div>
    <div class="card-b"><?= ts_chart_area($priceRows, 't', 'p', 'Litecoin price over time', $priceLabels, [
