...
<div class="card">
  ...
  <div class="card-b">
    ...
    <?php if ($showMiss): ?>
    <div class="mt-3"><div class="muted sub" style="display:inline-flex;align-items:center;gap:6px"><?= ts_icon('log-out') ?>Missing <span class="mono">(<?= commas($audit['missing']) ?>)</span></div>
    ...
    <?php endif; ?>
    <?php if ($showAdd): ?>
    <div class="mt-3"><div class="muted sub" style="display:inline-flex;align-items:center;gap:6px"><?= ts_icon('log-in') ?>Added <span class="mono">(<?= commas($audit['added']) ?>)</span></div>
    ...
    <?php endif; ?>
    <p class="pnote"><?= ts_icon('info') ?><span><b>Block health</b> = n / (n + excluded), the share of the high-fee transactions we predicted that the miner actually included (100% = nothing predicted was left out), measured against the fee-ordered ~1 vMB template we snapshotted<?php if ($lead !== null && $lead > 0): ?> ~<?= h(ts_dur_short($lead)) ?> before this block<?php endif; ?>.</span></p>
...
?>
<div class="card">
  <div class="card-h"><span><?= ts_icon('grid') ?>Transaction map</span> <span class="sub">page <?= $page + 1 ?> / <?= max(1, $pages) ?> · sized by vsize, shaded by fee rate</span></div>
  <div class="card-b"><?= ts_block_treemap($tmItems, 'Block ' . commas($bh) . ' transactions, page ' . ($page + 1)) ?>
    <div class="tm-legend"><span class="muted sub">Fee rate</span><?php foreach ([1, 4, 10, 25, 50, 100] as $r): ?><span class="tm-key" style="background:<?= h(ts_feerate_color((float) $r)) ?>"><?= $r >= 100 ? '100+' : $r ?></span><?php endforeach; ?><span class="muted sub">sat/vB · hover a cell for its txid</span></div>
  </div>
</div>
<?php endif; ?>
...
<div class="card">
  ...
  <div class="card-b nopad table-wrap">
    <table>
      ...
      <?php endforeach; ?>
      <?php if (!$txs): ?><tr><td colspan="4"><div class="empty"><?= ts_icon('inbox') ?><span><?= (int) $blk['height'] === 0 ? 'The genesis coinbase cannot be retrieved over RPC (a consensus rule), so it is not listed here.' : 'No transactions on this page.' ?></span></div></td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
